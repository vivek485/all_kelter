...
def gr(df):
    fig = go.Figure(
        data=[go.Candlestick(x=df.name, open=df.Open, close=df.Open, high=df.High, low=df.Low, name='Candelstick'),
...
async def getdata(session, stock):
    ...
    async with session.get(url, headers=headers) as response:
        try:
            ...
            final_df['signal1p'] = np.where(((final_df.buysig == 6) & (final_df.prbuysig != 6)), final_df['Low'],
                                            np.nan)
            final_df['signal2p'] = np.where(((final_df.sellsig == 3) & (final_df.prsellsig != 3)), final_df['High'],
                                            np.nan)
            # -----------------------------------------------------------------------------------------------------------------------
            ...
            st.title(stock)
            st.write(df_last + ' kelter_stratergy  ')
            st.plotly_chart(fig)
